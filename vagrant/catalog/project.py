...
import re
##### FAKE DATABASE
from database import categories, category, items, item
...
def newCategory():
	...
	# GET requests
		...
		if login_session.get('user_id') == True:
...
def newItem():
	...
	if request.method == 'POST':
		...
		# Checks to see if duplicate category
		if session.query(Item).filter_by(category_id=category_id).filter_by(name=name).first():
			# Flash duplicate category
	...
	# GET requests
		...
		if login_session.get('user_id') == True:
			...
			categories=session.query(Category).all()
			# Renders new category page
			return render_template('newItem.html', categories=categories, login_session=login_session)
		else:
			# Renders login page
			return redirect(url_for('login'))
...
def editItem(category_id, item_id):
	...
	if request.method == 'POST':
		...
		editItem.category_id = session.query(Category).filter_by(name=category).one().id
		...
		return render_template('editItem.html', categories=categories, category=category, item=item, login_session=login_session)
...
def deleteItem(category_id, item_id):
	# POST requests
	if request.method == 'POST':
		# Find Item database entry with name=category, update with form entry
...
def validate_email(email):
	...
	if not session.query(User).filter_by(email=email).first() and re.match(regex, email) != None:
		return True

def validate_username(username):
	regex = "(^([a-zA-Z0-9!@#$%^&*]{5,30}))"
	if not session.query(User).filter_by(username=username).first() and re.match(regex, username) != None:
		return True

def validate_passwords(password1, password2):
	regex = "(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[^a-zA-Z])(?=.*?[^a-zA-Z0-9]).{9,35}"
	if password1==password2 and re.match(regex, password1) != None:
		return True 
...
def signup():
	...
	if request.method == 'POST':
		...
		if validate_email(email) and validate_username(username) and validate_passwords(password, confirmPassword):
			...
			login_session['user_id'] = session.query(User).filter_by(username=username).one().id
...
def login():
	'''Create anti-forgery state token that will be used later for this session'''
	...
	if request.method == 'POST':
		...
		if sessionQuery.all() and bcrypt.check_password_hash(pw_hash, password):
...
def gconnect():
    ...
    # Compare id of token in credentials object with id returned by Google API Server
...
# if __name__ makes sure server only runs if script run from python interpreter,
# and not as imported code.
if __name__ == '__main__':
	app.config['TEMPLATES_AUTO_RELOAD'] = True
	# flash will use to create sessions for users.
	app.secret_key = 'super_secret_key'
	# Server reloads itself each time a load change, and gives helpful debugger.
	app.debug = True
